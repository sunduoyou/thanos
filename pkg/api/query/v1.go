...
func (qapi *QueryAPI) queryRange(r *http.Request) (interface{}, []error, *api.ApiError, func()) {
	...
	for i := range seriesStats {
		seriesStat := seriesStats[i]
		seriesMap["series"] = seriesStat.Series
		seriesMap["chunks"] = seriesStat.Chunks
		seriesMap["samples"] = seriesStat.Samples
		qapi.seriesStatsAggregator.Aggregate(seriesStat)
	}
	qapi.seriesStatsAggregator.Observe(time.Since(beforeRange).Seconds())
	...
	//print queryLog
	queryLogMap, shardInfoLogMap := getQueryStatsLog(qry.Stats(), shardInfo)

	qs = stats.NewQueryStats(qry.Stats())
	_ = qapi.logger.Log("", "timings", qs.Builtin().Timings, "samples", qs.Builtin().Samples, "series", seriesMap, "shard_info", shardInfo.String())
	return &queryData{
		ResultType: res.Value.Type(),
		Result:     res.Value,
		Stats:      qs,
